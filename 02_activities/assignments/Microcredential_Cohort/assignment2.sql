...
SELECT
	...
	product_qty_type,
    CASE
        WHEN TRIM(SUBSTR(product_name, INSTR(product_name, '-') + 1))
             IN ('Organic', 'Jar')
        THEN
...
SELECT
    v.vendor_name,
    p.product_name,
    SUM(5 * vi.original_price) AS total_money
FROM (
    SELECT DISTINCT
        vendor_id,
        product_id,
...
VALUES (
    6, 'Cut Zinnias Bouquet', 'large',  5, 'unit', CURRENT_TIMESTAMP
);
...
--QUERY 11
/* If using timestamp it will delete all records with same timestamp.
